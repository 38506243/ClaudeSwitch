...
class AppDelegate: NSObject, NSApplicationDelegate, NSMenuDelegate, NSControlTextEditingDelegate, NSTextFieldDelegate {
    ...
    @objc private func selectModel(_ sender: NSMenuItem) {
        ...
        for i in 0..<models.count {
            models[i].isActive = (models[i].id == modelId)
        }
        ...
        if let model = models.first(where: { $0.id == modelId }) {
            let token = model.apiToken.isEmpty ? defaultToken : model.apiToken
    ...
    private func showSheet(mode: SheetMode, existing: ModelItem?) {
        ...
        let fh: CGFloat = 24
        let gy: CGFloat = 38
        let startY: CGFloat = 360   // 内容顶部（510-460=50pt底部padding）
        ...
        sheetNameField.isEditable = (mode == .add)  // 添加模式可编辑，编辑模式只读
        sheetNameField.isEnabled = (mode == .add)
        sheetNameField.textColor = (mode == .add) ? .textColor : .secondaryLabelColor
        ...
        hintLbl.frame = NSRect(x: pad * 5, y: 70, width: 455, height: 30)
        ...
        if mode == .edit {
            ...
            delBtn.frame = NSRect(x: pad * 5, y: by, width: 100, height: 28)
